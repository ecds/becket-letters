...
class LettersByWorkOfArtMentioned extends Component {
  ...
  searchData = (searchTerms) => {
    ...
    axios.all([
      axios.get(this.props.apiUrl + '/search-entities?query=' + searchTerms + '&type=work_of_art')])
  ...
  getData = () => {
    axios.all([
      axios.get(this.props.apiUrl + '/entities?entity_type=work_of_art&items=50&page=' + this.state.page)])
      .then(axios.spread((getAllData) => {
        const data = getAllData.data.data;
        const pagination = getAllData.data.meta.pagination;
        this.setState({ pagination, data, isLoaded: true });
  ...
  render() {
    ...
    const meta = {
      title: 'Browse by Art',
      description: `Browse all letters by work of art mentioned`,
    };
...
export default LettersByWorkOfArtMentioned;
