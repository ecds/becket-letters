...
class LettersByTranslationsMentioned extends Component {
  ...
  searchData = (searchTerms) => {
    ...
    axios.all([
      axios.get(this.props.apiUrl+'/search-entities?query='+searchTerms+'&type=translating')])
  ...
  getData = () => {
    axios.all([
        axios.get(this.props.apiUrl+'/entities?entity_type=translating&items=50&page='+this.state.page)])
        .then(axios.spread((getAllData) => {
            const data = getAllData.data.data;
            const pagination = getAllData.data.meta.pagination;
            this.setState({ pagination, data, isLoaded: true });
  ...
  render() {
    var EntityList = this.state.data.map((entity) =>
        <tr key={entity.id}>
          <td>
            <Link to={{ pathname: `/translations/${entity.id}`, state: { id: entity.id} }}>
...
export default LettersByTranslationsMentioned;
