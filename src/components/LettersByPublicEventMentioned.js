...
class LettersByPublicEventMentioned extends Component {
  ...
  searchData = (searchTerms) => {
    ...
    axios.all([
      axios.get(this.props.apiUrl + '/search-entities?query=' + searchTerms + '&type=public_event')])
  ...
  getData = () => {
    axios.all([
      axios.get(this.props.apiUrl + '/entities?entity_type=public_event&items=50&page=' + this.state.page)])
      .then(axios.spread((getAllData) => {
        const data = getAllData.data.data;
        const pagination = getAllData.data.meta.pagination;
        this.setState({ pagination, data, isLoaded: true });
            ...
            <Form className="tab-search" onSubmit={this.intiateSearch} ref="form">
              <Form.Group>
                <div className="input-group mb-3">
                  ...
                  <Form.Control id="query" name="query" type="query" aria-label='query' placeholder="ex. 'Bastille Day'" />
                </div>
              </Form.Group>
            </Form>
          </Col>
...
export default LettersByPublicEventMentioned;
