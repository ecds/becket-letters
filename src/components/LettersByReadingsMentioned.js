...
class LettersByReadingsMentioned extends Component {
  ...
  getData = () => {
    axios.all([
      axios.get(this.props.apiUrl + '/entities?entity_type=reading&items=50&page=' + this.state.page)])
      .then(axios.spread((getAllData) => {
        const data = getAllData.data.data;
        const pagination = getAllData.data.meta.pagination;
        this.setState({ pagination, data, isLoaded: true });
            ...
            <Form className="tab-search" onSubmit={this.intiateSearch} ref="form">
              <Form.Group>
                <div className="input-group mb-3">
                  ...
                  <Form.Control id="query" name="query" type="query" aria-label='query' placeholder="ex. 'The Warden'" />
                </div>
              </Form.Group>
            </Form>
          </Col>
...
export default LettersByReadingsMentioned;
